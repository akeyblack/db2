...
class Worker(Process):
    ...
    def message_processing(self, msg_to_process):
        ...
        msg_tags = self.__db.hget(msg_to_process, "tags").decode('utf-8')
        ...
        if bool(random_bit()):
            ...
            self.__neo.create_message_relation(msg_owner, msg_rec, msg_to_process.decode('utf-8'), list(msg_tags.split(',')), False)
        else:
            self.__db.publish("events", f"Spam from {msg_owner} to {msg_rec}. Text: {msg_text}")
            self.__neo.create_message_relation(msg_owner, msg_rec, msg_to_process.decode('utf-8'),
                                               list(msg_tags.split(',')), True)
